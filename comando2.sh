...
sudo docker exec nginx-container sed -i 's|Welcome to nginx!|BIENVENIDO ALDAIR2|g; s|If you see this page, the nginx web server is successfully installed and working. Further configuration is required.|Esta es la página de prueba para verificar que Nginx está funcionando correctamente.|g; s|For online documentation and support please refer to nginx.org.|Documentación en línea disponible en nginx.org.|g; s|Commercial support is available at nginx.com.|Soporte comercial en nginx.com.|g; s|Thank you for using nginx.|Gracias por usar nginx.|g' /usr/share/nginx/html/index.html
